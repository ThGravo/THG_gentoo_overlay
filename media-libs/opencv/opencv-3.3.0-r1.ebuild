...
src_prepare() {
	#append-cxxflags "-isystem /opt/cuda/include"
...
#		-DWITH_NVCUVID=$(usex cuda)
		...
		-DCUDA_CUDART_LIBRARY=/opt/cuda/lib64/libcudart.so
		-DCUDA_NVCC_FLAGS:STRING='"-std=c++11 ";"-Wno-deprecated-gpu-targets ";"-Xcompiler ";"-D__CORRECT_ISO_CPP11_MATH_H_PROTO"'
		-DCUDA_TOOLKIT_ROOT_DIR="/opt/cuda"
		-DCUDA_TOOLKIT_INCLUDE="/opt/cuda/include"
	...
	if use contrib; then
		GLOBALCMAKEARGS+=(
			...
			-DBUILD_opencv_dnns_easily_fooled=$(usex contrib_dnn ON OFF)
			-DBUILD_opencv_dnn_modern=$(usex contrib_dnn ON OFF)
			-DTINYDNN_USE_OMP=ON
			-DTINYDNN_USE_AVX=ON
			-DTINYDNN_USE_TBB=ON
